...
impl CraneGame {
    fn solve(&mut self) -> String {
        for instruction in &self.instructions {
            // This could probably be optimized but I haven't even written my parser code yet!
            for _ in 0..instruction.quantity {
                let victim = self.crates[instruction.from - 1].pop_back().unwrap();
                self.crates[instruction.to - 1].push_back(victim);
            }
        }
        self.crates
            .iter()
